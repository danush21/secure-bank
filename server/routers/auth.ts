...
import { invalidateAllUserSessions, enforceSessionLimit, invalidateSession } from "@/lib/session";
...
export const authRouter = router({
  signup: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const { ssnEncrypted, ...userWithoutSSN } = user;
      return { user: { ...userWithoutSSN, password: undefined }, token };
    }),
  ...
  logout: publicProcedure.mutation(async ({ ctx }) => {
    ...
    if (ctx.user) {
      ...
      if (token) {
        sessionInvalidated = await invalidateSession(token);
        
        if (!sessionInvalidated) {
          console.warn(`Logout attempt with invalid/missing session token for user ${ctx.user.id}`);
          sessionInvalidated = true;
        }
      } else {
        console.warn(`Logout attempt without session token for user ${ctx.user.id}`);
        sessionInvalidated = true;
      }
    } else {
      sessionInvalidated = true;
    ...
    if (!sessionInvalidated) {
      throw new TRPCError({
        code: "INTERNAL_SERVER_ERROR",
        message: "Failed to logout - session invalidation failed",
      });
    }

    return { 
      success: true, 
      message: ctx.user ? "Logged out successfully" : "No active session to logout from" 
    };
  }),
});
